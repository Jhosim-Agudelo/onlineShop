...
$.getJSON('data/products.json', (data) => {
  ...
  for (let i = 0; i < data[idUrl].features.length; i++) {
  ...
  $('#add-to-cart-form button').click(() => {
    const tempsDelay = 5000;
    const ajoutProduit = $('#add-to-cart-form').serializeArray();
    const id = $.urlParam('id');

    let quantite = localStorage.getItem('qte');

    if (quantite == null) {
      localStorage.setItem('qte', 0);
      $('.count').text(1);
    }
    ...
    $('.count').text(quantite);
    $('#dialog').show().delay(tempsDelay).fadeOut();
  });
});
